...
export default function NouveauDevisPage(){
  ...
  useEffect(()=>{
    ...
    try{
      ...
      if(raw){
        ...
        const devis=list.find((d:any)=>d.id===devisId)
        if(devis){
          // Charger le client depuis snapshot ou localStorage
          if(devis.clientSnapshot){
            setClient({
              nom:devis.clientNom||devis.clientSnapshot.nom||'',
              adresse:devis.clientSnapshot.adresseFactLine1||'',
              email:devis.clientSnapshot.email||'',
              tel:devis.clientSnapshot.tel||'',
          ...
          if(devis.dateDevis) setDateDevis(devis.dateDevis.split('T')[0]||devis.dateDevis)
          if(devis.validite) setValidite(devis.validite)
  ...
  // Sauvegarder le devis dans localStorage à chaque modification
  const saveDevis=(updates:any={})=>{
    if(!devisId) return
    try{
      const raw=localStorage.getItem('batizo_devis')
