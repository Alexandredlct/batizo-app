...
const initChantiers = [
  { id:'c042', mois:'Avril 2026', client:'Martin Dupont', titre:'Rénovation salle de bain', adresse:'45 av. des Fleurs, Courbevoie 92400', tel:'06 12 34 56 78', vendeur:'Alexandre D.', montantDevis:4200, montantFacture:1260, statut:'attente', archive:false,
    docs:[
  ...
  { id:'c041', mois:'Avril 2026', client:'SCI Les Pins', titre:'Réfection façade + peinture', adresse:'12 rue Victor Hugo, Paris 75017', tel:'01 45 67 89 10', vendeur:'Emma S.', montantDevis:12800, montantFacture:9600, statut:'signe', archive:false,
    docs:[
  ...
  { id:'c040', mois:'Avril 2026', client:'Isabelle Renard', titre:'Pose parquet 45m²', adresse:'8 rue des Lilas, Levallois 92300', tel:'06 98 76 54 32', vendeur:'Alexandre D.', montantDevis:1950, montantFacture:0, statut:'attente', archive:false,
    docs:[
      { ref:'DEV-2026-040', type:'devis', montant:'1 950 €', date:'01/04/2026', statut:'attente' },
    ]
  },
  { id:'c039', mois:'Mars 2026', client:'SARL Bâti Pro', titre:'Rénovation bureaux 180m²', adresse:'5 rue du Commerce, Boulogne 92100', tel:'01 46 05 12 34', vendeur:'Emma S.', montantDevis:8600, montantFacture:0, statut:'refuse', archive:false,
    docs:[
      { ref:'DEV-2026-039', type:'devis', montant:'8 600 €', date:'28/03/2026', statut:'refuse' },
    ]
  },
  { id:'c038', mois:'Mars 2026', client:'M. Fontaine', titre:'Installation électrique complète', adresse:'22 bd Haussmann, Paris 75009', tel:'06 55 44 33 22', vendeur:'Alexandre D.', montantDevis:12800, montantFacture:12800, statut:'signe', archive:false,
    docs:[
...
const NavIcon = ({id}: {id:string}) => {
  if (id==='dashboard') return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{width:17,height:17}}><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
  if (id==='devis') return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{width:17,height:17}}><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
  if (id==='clients') return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{width:17,height:17}}><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{width:17,height:17}}><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg>
}
...
export default function DevisPage() {
  ...
  const getCount = (tab: string) => {
    if (tab === 'tous') return chantiers.filter(c => !c.archive).length
    if (tab === 'archive') return chantiers.filter(c => c.archive).length
    return chantiers.filter(c => !c.archive && c.statut === tab).length
  }
  ...
  const getMoisStats = (mois: string) => {
    ...
    const totalPaye = items.reduce((s, c) => s + c.montantFacture, 0)
    const aEncaisser = totalDevis - totalPaye
        ...
        <nav style={{flex:1,padding:'12px 8px',display:'flex',flexDirection:'column',gap:2}}>
          {navItems.map(n => (
            <a key={n.id} href={n.href}
              style={{display:'flex',alignItems:'center',gap:10,padding:'9px 10px',borderRadius:8,background:n.id==='devis'?'#f0fdf4':'none',color:n.id==='devis'?G:'#555',fontWeight:n.id==='devis'?600:400,fontSize:13,textDecoration:'none',transition:'background 0.15s'}}
            ...
            {tabs.map(t => {
              ...
              return (
                <button key={t} onClick={() => setActiveTab(t)}
                  ...
                  {tabLabels[t]}
                  {count > 0 && <span style={{background:activeTab===t?G:'#e5e7eb',color:activeTab===t?'#fff':'#888',fontSize:10,fontWeight:700,padding:'1px 6px',borderRadius:10,minWidth:16,textAlign:'center'}}>{count}</span>}
                </button>
              )
            })}
          </div>
                      ...
                      <div style={{display:'flex',alignItems:'flex-start',gap:12,flex:1,minWidth:0}}>
                        ...
                        <div>
                          ...
                          <div style={{fontSize:12,color:'#666',marginBottom:2}}>
                            Total devisé : <strong>{fmt(c.montantDevis)} HT</strong> · Total facturé : <strong style={{color:c.montantFacture===0?'#aaa':G}}>{fmt(c.montantFacture)} HT</strong>
                          </div>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{position:'relative',display:'inline-flex',alignItems:'center',gap:4}}>
                          ...
                          <button
                            ...
                            style={{width:20,height:20,borderRadius:'50%',border:`1px solid ${BD}`,background:'#f9fafb',fontSize:11,fontWeight:700,color:'#888',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                            ?
                          </button>
                          {/* Tooltip redesigné */}
                          {tooltip === c.id && (
                            <div style={{position:'absolute',right:0,top:30,background:'#fff',color:'#333',fontSize:12,padding:'10px 14px',borderRadius:10,zIndex:200,width:240,lineHeight:1.6,boxShadow:'0 4px 20px rgba(0,0,0,0.15)',border:`1px solid ${BD}`}}>
                              <div style={{position:'absolute',top:-6,right:12,width:12,height:12,background:'#fff',border:`1px solid ${BD}`,borderRight:'none',borderBottom:'none',transform:'rotate(45deg)'}}></div>
                    ...
                    {expanded[c.id] && (
                      <div style={{borderTop:`1px solid ${BD}`}}>
                        <table style={{width:'100%',borderCollapse:'collapse'}}>
                          ...
                          <tbody>
                            {c.docs.map((d, di) => (
                              <tr key={di} style={{borderTop:`1px solid ${BD}`,transition:'background 0.15s',cursor:'pointer'}}
                                ...
                                <td style={{padding:'10px 16px',textAlign:'center'}}>
                                  <button style={{background:'none',border:'none',cursor:'pointer',color:'#aaa',fontSize:18,lineHeight:1}}>⋯</button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                        ...
                        <div style={{position:'relative'}}>
                          <div onClick={() => setDocMenu(docMenu===c.id ? null : c.id)}
                            style={{padding:'10px 16px',borderTop:`1px solid ${BD}`,display:'flex',alignItems:'center',gap:6,fontSize:13,color:G,cursor:'pointer',background:'#f9fafb',transition:'background 0.15s',userSelect:'none'}}
                            onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.background='#f0fdf4'}
                          ...
                          {docMenu === c.id && (
                            <div style={{background:'#fff',border:`1px solid ${BD}`,borderRadius:8,boxShadow:'0 4px 16px rgba(0,0,0,0.1)',overflow:'hidden',marginTop:2,zIndex:100}}>
                              {docMenuItems.map(item => (
                                <div key={item} onClick={() => setDocMenu(null)}
                                  style={{padding:'10px 16px',fontSize:13,cursor:'pointer',color:'#333',transition:'background 0.15s,color 0.15s'}}
                                  onMouseEnter={e => { const d = e.currentTarget as HTMLDivElement; d.style.background='#f0fdf4'; d.style.color=G }}
                                  onMouseLeave={e => { const d = e.currentTarget as HTMLDivElement; d.style.background=''; d.style.color='#333' }}>
                                  {item}
                                </div>
                              ))}
                            </div>
