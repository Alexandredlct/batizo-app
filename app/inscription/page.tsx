          ...
          <div style={{background:'#fff',borderRadius:'16px',padding:'2rem',boxShadow:'0 4px 24px rgba(0,0,0,0.08)',border:'1px solid #999'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
                ...
                <div><label style={{display:'block',fontSize:'13px',fontWeight:'500',marginBottom:'6px'}}>Nom *</label><input style={inputStyle} placeholder="Dupont" value={form.nom} onChange={e => setForm({...form, nom: e.target.value})}/></div>
              </div>
              <div><label style={{display:'block',fontSize:'13px',fontWeight:'500',marginBottom:'6px'}}>Email professionnel *</label><input type="email" style={inputStyle} placeholder="vous@email.fr" value={form.email} onChange={e => setForm({...form, email: e.target.value})}/></div>
              <div><label style={{display:'block',fontSize:'13px',fontWeight:'500',marginBottom:'6px'}}>Téléphone</label><input type="tel" style={inputStyle} placeholder="06 12 34 56 78" value={form.telephone} onChange={e => setForm({...form, telephone: e.target.value})}/></div>
              <div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              ...
              <div>
                ...
                <select style={{...inputStyle}} value={form.metier} onChange={e => setForm({...form, metier: e.target.value})}>
                  <option>Électricité</option><option>Plomberie</option><option>Peinture</option><option>Carrelage</option><option>Menuiserie</option><option>Multi-corps</option><option>Maçonnerie</option><option>Autre</option>
                </select>
              </div>
              <div><label style={{display:'block',fontSize:'13px',fontWeight:'500',marginBottom:'6px'}}>SIRET (optionnel)</label><input style={inputStyle} placeholder="853 572 014 000 19" value={form.siret} onChange={e => setForm({...form, siret: e.target.value})}/></div>
              <div>
                ...
                <div style={{display:'flex',gap:'8px',flexWrap:'wrap' as const}}>
                  {[['solo','Solo'],['small','2-5 pers.'],['medium','6-15 pers.'],['large','15+']].map(([val, label]) => (
                    <button key={val} type="button" onClick={() => setTeamSize(val)} style={{padding:'6px 14px',borderRadius:'6px',fontSize:'13px',cursor:'pointer',border:`1px solid ${teamSize === val ? green : '#e5e7eb'}`,background: teamSize === val ? '#f0fdf4' : '#fff',color: teamSize === val ? green : '#555',fontWeight: teamSize === val ? '600' : '400'}}>{label}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <select style={{...inputStyle}} value={form.comment} onChange={e => setForm({...form, comment: e.target.value})}>
                  <option>Bouche-à-oreille</option><option>Google</option><option>Réseaux sociaux</option><option>Salon professionnel</option><option>Article / Presse</option><option>Autre</option>
                </select>
              </div>
