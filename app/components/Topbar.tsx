...
export default function Topbar({ title }: { title: string }) {
  ...
  return (
    <div style={{ height: 60, background: '#fff', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', flexShrink: 0, position: 'relative', zIndex: 100 }}>
      {/* Titre */}
      <div style={{ fontSize: 16, fontWeight: 700, color: '#111', minWidth: 120 }}>{title}</div>

      {/* Barre de recherche centrée */}
      <div style={{ position: 'relative', width: '100%', maxWidth: 480, margin: '0 24px' }} onClick={e => e.stopPropagation()}>
        <svg style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2">
        ...
        <input
          ...
          style={{ width: '100%', padding: '9px 32px 9px 36px', border: '1px solid #d1d5db', borderRadius: 10, fontSize: 13, outline: 'none', color: '#111', background: '#f9fafb', boxSizing: 'border-box' as const, transition: 'all 0.15s' }}
        />
        {search && (
          <button onClick={() => { setSearch(''); setOpen(false) }}
            style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#aaa', fontSize: 18, lineHeight: 1 }}>×</button>
        )}

        {/* Dropdown */}
        {open && search.length >= 2 && (
          <div style={{ position: 'absolute', top: '110%', left: 0, right: 0, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.15)', maxHeight: 420, overflowY: 'auto' }}>
            {results.length === 0 ? (
              <div style={{ padding: '20px', textAlign: 'center' as const, fontSize: 13, color: '#888' }}>
              ...
              <div key={cat}>
                ...
                {items.map((item: any, i: number) => (
                  <a key={i} href={item.href}
                    ...
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#aaa" strokeWidth="2"><polyline points="9 18 15 12 9 6" /></svg>
                  </a>
                ))}
              </div>
            ))}
            <div style={{ padding: '8px 14px', fontSize: 11, color: '#aaa', textAlign: 'center' as const, borderTop: '1px solid #f3f4f6' }}>
      ...
      {open && <div onClick={() => setOpen(false)} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: -1 }} />}
    </div>
  )
}
